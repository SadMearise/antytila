...
if (audio) {
	...
	const tracksDurationDOM = document.querySelectorAll('.audio-item__track-duration');
	...
	const progressArea = document.querySelector('.audio-progress__line-container');
	...
	function changeIcon(trackIndex, addClass, removeClass) {
		...
		playBtns[trackIndex].insertAdjacentHTML('afterbegin', `<use xlink:href="../img/icons/icons.svg#${addClass}"></use>`)
	}
	...
	function prevSong(trackIndex) {
		if (trackIndex == 0) {
			pauseSong(trackIndex + 1);
			trackIndex--;
			trackIndex < 0 ? trackIndex = audioItems.length : trackIndex = trackIndex;
		} else {
			if (!playBtns[trackIndex].classList.contains('svg-play-dims')) {
				pauseSong(trackIndex);
			}
			trackIndex--;
			trackIndex < 1 ? trackIndex = audioItems.length : trackIndex = trackIndex;
		}
		loadSong(trackSrcs[trackIndex - 1], trackNames[trackIndex - 1]);
		playSong(trackIndex);
	...
	playBtns.forEach(function (playBtn) {
		playBtn.closest('.icons__item').addEventListener("click", function (e) {
			...
			if (e.target.closest('.audio-block__icons')) {
				if (prevIndex == 0) {
					prevIndex = 1;
					playBtn.classList.contains('svg-play-dims') ? playSong(trackIndex + 1) : pauseSong(trackIndex + 1);
				} else {
					playBtn.classList.contains('svg-play-dims') ? playSong(prevIndex) : pauseSong(prevIndex);
				}
			} else {
				const activeAudioSrc = playBtn.closest('.audio-item').childNodes[2].nextSibling.dataset.src;
	...
	audio.addEventListener("timeupdate", (e) => {
		...
		let musicCurrentTime = document.querySelector('.audio-progress__current-time');
		audio.addEventListener("loadeddata", () => {
			let mainAdDuration = audio.duration;
			let totalMin;
			Math.floor(mainAdDuration / 60) > 10 ? totalMin = Math.floor(mainAdDuration / 60) : totalMin = '0' + Math.floor(mainAdDuration / 60);
		...
		let currentMin;
		Math.floor(currentTime / 60) > 10 ? currentMin = Math.floor(currentTime / 60) : currentMin = '0' + Math.floor(currentTime / 60);
